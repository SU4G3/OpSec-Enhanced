...
public class OpsecConfigScreen extends Screen {
    ...
    *///?}
    ...
    private void openImportDialog() {
        ...
        new Thread(() -> {
            try {
                java.nio.file.Path file = aurick.opsec.mod.util.NeoCompat.getConfigDir().resolve("opsec-accounts-import.json");
    ...
    private Tab createWhitelistTab(SpoofSettings settings) {
        ...
        if (OpsecConfig.EXPLOIT_PREVENTER_LOADED) {
            ...
            if (settings.getWhitelistMode() == SpoofSettings.WhitelistMode.AUTO) {
                List<IModInfo> whitelistableMods = getWhitelistableMods();
                for (IModInfo mod : whitelistableMods) {
                ...
                widgets.add(new ToggleAllRowWidget(
                    () -> {
                        for (IModInfo m : whitelistableMods) {
                            settings.getWhitelistedMods().add(m.getModId());
                        }
                        config.save();
                        refreshScreen();
                    },
    ...
    /**
     * Every installed top-level mod, listed as-is so any mod is whitelistable.
     * Unlike the Fabric build, this doesn't exclude JarJar'd nested mods —
     * {@code IModInfo} has no NeoForge equivalent of {@code getContainingMod()}
     * exposed at this level; a nested mod just shows up as its own entry too.
     */
    private static List<IModInfo> getWhitelistableMods() {
        return net.neoforged.fml.ModList.get().getMods().stream()
            .filter(mod -> !ModRegistry.PLATFORM_MODS.contains(mod.getModId()))
